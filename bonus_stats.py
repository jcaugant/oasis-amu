...
liste_hal = []
url_total = f"https://api.archives-ouvertes.fr/search/?q=*%3A*&fq=publicationDateY_i:2024&fq=structId_i:{portail}&wt=json"
url_file = f"https://api.archives-ouvertes.fr/search/?q=*%3A*&fq=publicationDateY_i:2024&fq=structId_i:{portail}&wt=json&fq=submitType_s:file"
if link:
  url_notice = f"https://api.archives-ouvertes.fr/search/?q=*%3A*&rows=10000&fq=publicationDateY_i:2024&fq=structId_i:{portail}&wt=json&fq=(submitType_s:notice%20NOT%20openAccess_bool:(true))&fl=halId_s"
else :
  url_notice = f"https://api.archives-ouvertes.fr/search/?q=*%3A*&rows=10000&fq=publicationDateY_i:2024&fq=structId_i:{portail}&wt=json&fq=submitType_s:notice&fl=halId_s"
...
try :
  ...
  st.write(f"Pour l'année 2024, votre structure a déposé {file} documents avec texte intégral et {notice} notices")
  # st.write(f"Votre pourcentage de publications avec texte intégral déposées dans HAL pour l'année 2024 est donc de : ", int((file/total)*100), "%")
  st.write("Votre pourcentage de publications avec texte intégral déposées dans HAL pour l'année 2024 est donc de : ")
